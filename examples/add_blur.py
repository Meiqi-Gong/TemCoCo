...
def random_anisotropic_gaussian_kernel(batch=1, kernel_size=21, lambda_min=0.2, lambda_max=4.0):
    theta = torch.rand(batch).to(device) / 180 * math.pi
...
def stable_anisotropic_gaussian_kernel(kernel_size=21, theta=0, lambda_1=0.2, lambda_2=4.0):
    theta = torch.ones(1).to(device) * theta * math.pi
...
class bicubic(nn.Module):
    ...
    def contribute(self, in_size, out_size, scale):
        ...
        x0 = torch.arange(start=1, end=out_size[0] + 1).to(torch.float32).to(device)
        x1 = torch.arange(start=1, end=out_size[1] + 1).to(torch.float32).to(device)
        ...
        indice0 = left0.unsqueeze(1) + torch.arange(start=0, end=P).to(torch.float32).unsqueeze(0).to(device)
        indice1 = left1.unsqueeze(1) + torch.arange(start=0, end=P).to(torch.float32).unsqueeze(0).to(device)
        ...
        indice0 = torch.min(torch.max(torch.FloatTensor([1]).to(device), indice0), torch.FloatTensor([in_size[0]]).to(device)).unsqueeze(0)
        indice1 = torch.min(torch.max(torch.FloatTensor([1]).to(device), indice1), torch.FloatTensor([in_size[1]]).to(device)).unsqueeze(0)
...
def add_stripe(im):
    stdN_G = torch.FloatTensor(im.size(0)).uniform_(10, 15)  # 控制高斯噪声强度
    mean_G = torch.zeros(im.size())  # 均值为0的高斯噪声
    noise_G = torch.normal(mean_G, stdN_G)

    beta = torch.FloatTensor(im.size(-1)).uniform_(8, 8)  # 控制条纹噪声强度
    mean_col = torch.zeros((im.size(0), im.size(-1)))  # 均值为0的条纹噪声
...
class SRMDPreprocessing(object):
    ...
    def __call__(self, hr_tensor, random=False):
...
def VI_Degradation(input_folder, output_folder):
    # 获取目录中的所有图片文件并自然排序
    folder_list = os.listdir(input_folder)
    folder_list = natsorted(folder_list)

    for sub_folder in folder_list:  
        # if sub_folder == '0113_1631':      
        sig =  random.uniform(0.5, 3.5)
        theta = random.uniform(0.5, 3.5)
        preprocessor = SRMDPreprocessing(
                        scale=1,  # 缩放因子
                        mode='bicubic',  # 选择插值模式
                        kernel_size=12,  # 模糊核大小
                        blur_type='iso_gaussian',  # 模糊类型
                        sig=sig,  # 高斯模糊标准差
                        sig_min=1.2,  # 最小标准差
                        sig_max=2.5,  # 最大标准差
                        lambda_1=0.2,  # 非各向同性高斯模糊参数
                        lambda_2=2.5,  # 非各向同性高斯模糊参数
                        theta=theta,  # 非各向同性高斯模糊参数
                        lambda_min=0.2,  # 非各向同性高斯模糊参数
                        lambda_max=2.5,  # 非各向同性高斯模糊参数
                        noise=0  # 噪声水平
                    )
        ...
        for item in file_bar:
            image_path = os.path.join(input_folder, sub_folder, item)
            ...
            # 处理图像
            lr_blured, _ = preprocessor(image_tensor, random=False)

            # 保存处理后的图像
            # output_path = os.path.join(output_folder, name + '_Blur' + ext)
            output_path = os.path.join(output_folder, sub_folder, item)
            output_image = transforms.ToPILImage()(lr_blured[0].byte())  
            os.makedirs(os.path.dirname(output_path), exist_ok=True)          
            output_image.save(output_path)
            file_bar.set_description("{} | {}".format(sub_folder, item))

if __name__ == '__main__':
    # 设定目录路径
    # input_folder = "/home/whu/HDD_16T/timer/gmq/video/ours/HDO2/test/vi" 
    # output_folder = "/home/whu/HDD_16T/timer/gmq/video/ours/HDO2/test/vi_blur"
    # input_folder = "/home/whu/HDD_16T/timer/gmq/video/ours/Video0117/VI_en" 
    # output_folder = "/home/whu/HDD_16T/timer/gmq/video/ours/Video0117/VI_blur_new"
    # input_folder = "/home/whu/HDD_16T/timer/gmq/video/TemCoCo/test_ST_select/VI" 
    # output_folder = "/home/whu/HDD_16T/timer/gmq/video/TemCoCo/test_ST_select/VI_blur"
    input_folder = "/media/wit/whu/timer/gmq/video/TemCoCo/MS3V/all_with_LE/train/VI"
    output_folder = "/media/wit/whu/timer/gmq/video/TemCoCo/MS3V/all_with_LE/train/VI_blur_new"
    VI_Degradation(input_folder, output_folder)
