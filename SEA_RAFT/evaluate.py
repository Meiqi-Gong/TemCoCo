...
@torch.no_grad()
def validate(args, model, image1, image2):
    """ Peform validation using the Spring (val) split """
    ...
    flow, info = calc_flow(args, model, image1.cuda(), image2.cuda())
    flow_vis = flow_to_image(flow[0].permute(1, 2, 0).cpu().numpy(), convert_to_bgr=True)
    cv2.imwrite(f"flow_final7.jpg", flow_vis)
    return flow
...
def eval(args, image1, image2):
    ...
    model.eval()
    with torch.no_grad():
        flow = validate(args, model, image1, image2)
        return flow
