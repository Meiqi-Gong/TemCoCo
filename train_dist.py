...
print(f"local_rank: {local_rank}, device_count: {device_count}")
...
vgg = vgg19(pretrained=True).features.to(local_rank)

DINOv2_model = torch.hub.load('/home/whu/HDD_16T/timer/gmq/video/dinov2-main/', 'dinov2_vitl14',source='local').cuda()
DINOv2_model.eval()
vgg.eval()
...
FusionNet = torch.nn.SyncBatchNorm.convert_sync_batchnorm(FusionNet)
FusionNet = DDP(FusionNet, device_ids=[local_rank])
Fusion_loss = Fusion_loss().cuda()
optimizer = optim.Adam(FusionNet.parameters(), lr=opt['train']['optim_g']['lr'], betas=(0.9, 0.999), eps=1e-8)

def calcu_loss(fused, gt_ir, gt_vi, ir_feat, vi_feat):#, rand1, rand2, select):
    ...
    # return total_loss, loss_fusion, loss_dict
...
for epoch in range(start_epoch, total_epochs + 1):
    ...
    while train_data is not None:
        ...
        if iter>0:
            ...
            # fused, ir_feat, vi_feat = model(gt_ir, gt_vi)
