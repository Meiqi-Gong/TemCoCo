...
class Conv3DModule(nn.Module):
    def __init__(self, in_channels=3, out_channels=64, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1)):
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_chs, activation='relu', batch_norm=False):
        ...
        op = []
        for i in range(1):
            op.append(Separable3DConv(in_chs, in_chs))
            if batch_norm:
...
class Separable3DConv(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        super(Separable3DConv, self).__init__()
        # 2D 空间卷积 (Height × Width)
        self.spatial_conv = Conv3DModule(in_channels, out_channels, kernel_size=(1,3,3), stride=(1,stride,stride), padding=(0,1,1))
        self.depth_conv = Conv3DModule(out_channels, out_channels, kernel_size=(3,1,1), stride=1, padding=(1,0,0))
        ...
        # self.spatial_conv = nn.Conv2d(in_channels, out_channels, kernel_size=(3, 3), padding=1)
        # 1D 深度卷积 (Depth)
        # self.depth_conv = nn.Conv1d(out_channels, out_channels, kernel_size=3, padding=1)

    def forward(self, x):
        batch, channels, t, height, width = x.shape
        x = self.spatial_conv(x)
        x = self.depth_conv(x)  # Apply 1D depth convolution
        return x  # Reshape to original format
...
#         self.spatial_conv = Conv3DModule(in_channels, out_channels, kernel_size=(1,3,3), stride=(1,stride,stride), padding=(0,1,1))
#         # 2D 空间卷积 (Height × Width)
#         # self.spatial_conv = nn.Conv2d(in_channels, out_channels, kernel_size=(3, 3), stride=stride, padding=1)
#         # # 1D 深度卷积 (Depth)
#         # self.depth_conv = nn.Conv2d(out_channels, out_channels, kernel_size=(3, 3), padding=1)
#         # self.depth_conv = nn.Conv1d(out_channels, out_channels, kernel_size=3, padding=1)
...
#         # x = rearrange(x, "bt c h w-> bt c (h w)")
#         # x = self.depth_conv(x)  # Apply 1D depth convolution
#         # x = rearrange(x, "(b t) c h w -> b c t h w", b=batch)
#         # x = rearrange(x, "(b t) c (h w) -> b c t h w", b=batch, h=height)
#         return x  # Reshape to original format
...
def conv3x3_3d(in_channels, out_channels, stride=1):
    return nn.Conv3d(in_channels, out_channels, kernel_size=(3,3,3), stride=stride, padding=1, bias=True)
...
def conv_resblock_two(in_channels, out_channels, stride=1): 
    return nn.Sequential(Separable3DConv(in_channels, out_channels, stride), ResBlock(out_channels), ResBlock(out_channels))

def conv_resblock_one(in_channels, out_channels, stride=1):
    return nn.Sequential(Separable3DConv(in_channels, out_channels, stride), ResBlock(out_channels))
